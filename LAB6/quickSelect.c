//CHOOSE KTH SMALLEST ELEMENT OF ARRAY BY CHOOSING A PIVOT AND PARTITITONING
#include <stdio.h>
#include <stdlib.h>
...
int quickSelect(int* arr, int n, int k){
    ...
    int lo = 0;
    int hi = n-1;
    int pInd = hoarePartititon(arr, lo, hi, piv);

    if(k>pInd+1){
        return quickSelect(arr + pInd+1, n-pInd-1, k-pInd-1);
    }
    else if (k<= pInd){
        return quickSelect(arr, pInd, k);
    }
    else{
        return arr[pInd];
    }
