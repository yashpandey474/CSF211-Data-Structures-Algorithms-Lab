...
int strongly_connected_components(Graph* G){
    ...
    recDFS(G);  //DFS OF ORIGINAL GRAPH


    graph_node descending_f_indices[G->V]; //ARRAY WITH VERTICES IN SORTED FINISHING TIME

    for(int i = 0; i<G->V; i++){
        descending_f_indices[i].data = i;
        descending_f_indices[i].f = G->vertices[i].finishing_time;
    }

    //SORT THE VERTICES IN DESCENDING ORDER OF FINISHING TIME
    for(int i = 0; i<G->V; i++){
        for(int j = i+1; j<G->V; i++){
            if(descending_f_indices[i].finishing_time < descending_f_indices[i].finishing_time){
                graph_node temp = descending_f_indices[i];
                descending_f_indices[i] = descending_f_indices[j];
                descending_f_indiced[j] = temp;
            }
    
        }
    }

    //GET TRANSPOSE OF GRAPH: WITH ALL EDGES REVERSED
    Graph* Gt = get_transpose();
    for(int i = 0; i<G->V; i++){
        Gt->vertices[i].c = WHITE;
    }

    int num_scc = 0; //NUMBER OF SCCs
    time = 0;
    printf("\n");
